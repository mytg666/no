...
const CSP = `
  default-src 'self';
  connect-src 'self' ws://:30444 blob: http: https: ${APP_ENV === 'development' ? 'wss:' : ''};
  script-src 'self' 'wasm-unsafe-eval' https://t.me/_websync_ https://telegram.me/_websync_;
  style-src 'self' 'unsafe-inline';
  img-src 'self' data: blob: http: https: https://ss3.4sqi.net/img/categories_v2/
...
function isGitRepository(): boolean {
  try {
    ...
    // Try to check if HEAD exists
    execSync('git rev-parse --verify HEAD', { stdio: 'ignore', cwd: __dirname });
    return true;
  } catch {
    return false;
  }
...
function getGitMetadata() {
  // Return default values if not a Git repository or no commits
  if (!isGitRepository()) {
  ...
  try {
    ...
    try {
      ...
      branch = HEAD || gitBranch || 'unknown';
    } catch (error) {
      branch = HEAD || 'unknown';
    }
    
    try {
      commit = gitRevisionPlugin.commithash()?.substring(0, 7);
    } catch (error) {
      commit = 'unknown';
    }
    ...
    // Handle case when Git is not initialized or has no commits
    return { branch: HEAD || 'unknown', commit: 'unknown' };
  }
}
